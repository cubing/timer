...
import {set8Perm, get8Perm, circle, rn, rndEl} from "../lib/mathlib";
...
function FullCube_randomCube(indice?: any) {
...
let ranShape_$clinit = false;
function Shape_$clinit() {
	if (ranShape_$clinit) {
		return;
	}
	ranShape_$clinit = true;
...
let ranSquare_$clinit = false
function Square_$clinit() {
	if (ranSquare_$clinit) {
		return;
	}
	ranSquare_$clinit = true;
	SquarePrun = [];
	Square_TwistMove = [];
	Square_TopMove = [];
	Square_BottomMove = [];
...
function CSPInit() {
	...
	for (var csp = 0; csp < cspcases.length; csp++) {
		var curCases: number[] = [cspcases[csp] as number];
...
function square1SolverGetRandomScramble(type?: any, length?: any, cases?: any) {
	Shape_$clinit();
	Square_$clinit();
...
function square1CubeShapeParityScramble(type?: any, length?: any, cases?: any) {
	...
	Square_$clinit();
	CSPInit();
	var idx = rndEl(cspcases[fixCase(cases, cspprobs)]);
...
export {
	square1SolverGetRandomScramble as getRandomScramble
};
