...
function load(identifier: string) {
  (document.querySelector(`#${identifier}`) as HTMLInputElement).value =
...
function setupField(identifier: string) {
  load(identifier);
  (
    document.querySelector(`#${identifier}`) as HTMLInputElement
  ).addEventListener("change", save.bind(save, identifier));
}
